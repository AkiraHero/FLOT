...
def eval_model_with_vis(scene_flow, dataset):
    # Init.
    running_epe = 0
    running_outlier = 0
    running_acc3d_relax = 0
    ...
    from flot.tools.ros_publisher import MinimalPublisher
    ...
    inx = 811 
    for i in range(inx, len(dataset)):
        data_item = dataset[inx]
        batch = Batch([data_item])
        batch = batch.to(device, non_blocking=True)
        ...
        previous_pcl = data_item['sequence'][0][0].cpu().numpy()
        current_pcl = data_item['sequence'][1][0].cpu().numpy()
        flows_gt = data_item['ground_truth'][1][0].cpu().numpy()
        predicted_flows = est_flow[0].cpu().numpy()
        
        print("===========frm inx = {}".format(inx))
        publisher.publish_cloud_data("previous", previous_pcl)
        publisher.publish_cloud_data("current", current_pcl)
        
        current_plus_flow_esti = previous_pcl + predicted_flows[:, :3] * 0.1
        publisher.publish_cloud_data("cur_in_pre_estimation", current_plus_flow_esti)
        
        current_plus_flow_gt = previous_pcl + flows_gt[:, :3] * 0.1
        publisher.publish_cloud_data("cur_in_pre_gt", current_plus_flow_gt)
        ...
        running_acc3d_strict += acc3d_strict
        print(
        "EPE3D;{0:e};Outlier;{1:e};ACC3DR;{2:e};ACC3DS;{3:e};".format(
            EPE3D,
            outlier,
            acc3d_relax,
            acc3d_strict,
...
def my_main(dataset_name, max_points, path2ckpt, test=False):
    ...
    if dataset_name.split("_")[0].lower() == "HPLFlowNet".lower():
        ...
        elif dataset_name.split("_")[1].lower() == "waymo".lower():
            path2data = "/home/xlju/data/waymo_sf_processed/"
            # mode = "test" if test else "val"
            mode = "train"
            from flot.datasets.waymo_dataset import WaymoDataset as CurrentDataset
            # assert mode == "val" or mode == "test", "Problem with mode " + mode
            dataset = CurrentDataset(root_dir=path2data, nb_points=max_points, mode=mode)
        else:
    ...
    eval_model_with_vis(scene_flow, dataset)
